/* =========================================================================
   ...
   Sections in this file:
     ...
     2. Date helpers
     3. Generic custom-select widget (reused for every dropdown on page)
     4. Class-section performance bar chart (JS-plotted, filter driven)
     5. Term 2 exam calendar (render, navigation, sessions list)
     6. Modal: Add / edit exam calendar event
     7. Modal: Full day schedule ("View all")
     8. Grade & class structure (grade cards, class rows, inline editor)
     9. Teacher assignment field (dropdown + hover workload preview)
    10. Curriculum subjects (cards + inline editor)
    11. Modal: Add a grade
    12. Generic modal open/close (shared by all modals; also owns the two
        delete-confirmation modals, whose markup lives in index.html now)
    13. Shared chart tooltip helper
    14. App bootstrap
   Sidebar behaviour lives entirely in shared/sidebar.js — not duplicated
   here (see shared/sidebar.js).
...
(function () {
  ...
  /* =======================================================================
     3. GENERIC CUSTOM-SELECT WIDGET
     -------------------------------------------------------------------------
  ...
  /* =======================================================================
     4. CLASS-SECTION PERFORMANCE BAR CHART
     -------------------------------------------------------------------------
  ...
  function renderExamSessionsList() {
    ...
    if (dayEvents.length) {
      ...
      sessionsEl.innerHTML = `
        <div class="c-calendar__day-empty" style="display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 160px; padding: 1.25rem 1rem; text-align: center; border-radius: 12px; border: 1px dashed rgba(255,255,255,0.2); background: rgba(255,255,255,0.04);">
          <p style="color: rgba(255,255,255,0.7); font-size: 0.875rem; margin: 0 0 0.75rem;">No events scheduled for ${formatLongDate(state.selectedExamDate)}.</p>
          <button type="button" class="j-open-event-editor-btn" style="border: 1px dashed rgba(255, 255, 255, 0.6); border-radius: 8px; background: rgba(255, 255, 255, 0.1); padding: 0.75rem 1.5rem; cursor: pointer; color: #EA8913; font-weight: 600; font-size: 0.875rem; display: inline-flex; align-items: center; gap: 8px;" onmouseover="this.style.background='rgba(255, 255, 255, 0.18)'" onmouseout="this.style.background='rgba(255, 255, 255, 0.1)'">
            <span>Add event</span>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#EA8913" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 5v14M5 12h14"/></svg>
          </button>
        </div>`;
      sessionsEl.querySelector('.j-open-event-editor-btn')?.addEventListener('click', () => {
        openExamEditorModal(null);
  ...
  /** Shared by wireTeacherField + wireSubjectTeacherField — both popovers
   *  show the exact same "workload preview" card on hover/focus, so it's
   *  built once here instead of twice inline. */
  function renderWorkloadPreviewInto(popover, teacher) {
  ...
  /** Shared by wireTeacherField + wireSubjectTeacherField — the "pick a
   *  teacher" option-list markup is identical between the two; only the
   *  click behaviour differs, so that part stays local to each caller. */
  function renderTeacherOptionsHtml(orderedStaff, selectedValue) {
    let html = `<ul class="c-teacher-menu" role="listbox">`;
    html += `
      <li role="option" aria-selected="${!selectedValue}">
        <button type="button" class="c-teacher-menu__option j-teacher-option ${!selectedValue ? 'c-is-selected' : ''}" data-teacher-name="">
          Assignment pending
          ${!selectedValue ? '<svg class="c-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg>' : ''}
        </button>
      </li>`;
    orderedStaff.forEach((teacher) => {
      const isSelected = teacher.name === selectedValue;
      html += `
  ...
  function wireTeacherField(rootEl, draftKey) {
    ...
    function renderPreview(teacher) { renderWorkloadPreviewInto(popover, teacher); }
    ...
    function renderMenu() {
      ...
      popover.innerHTML = renderTeacherOptionsHtml(orderedStaff, value) +
        `<div class="c-workload-preview j-workload-preview c-workload-preview--${placement}"></div>`;
  ...
  /* =======================================================================
     12. GENERIC MODAL OPEN / CLOSE (shared by every modal on this page)
     ======================================================================= */

  /** Generic confirm-delete dialog. The shell (icon, layout, buttons) is
   *  static markup in index.html (#j-modal-confirm-delete) — this only
   *  fills in the three bits that actually vary per call. */
  function showConfirmDeleteModal({ title, description, buttonText, onConfirm }) {
    const layer = document.getElementById('j-modal-confirm-delete');
    document.getElementById('j-confirm-delete-title').textContent = title;
    document.getElementById('j-confirm-delete-description').textContent = description;

    const confirmBtn = document.getElementById('j-confirm-delete-confirm');
    confirmBtn.textContent = buttonText;
    const freshBtn = confirmBtn.cloneNode(true); // drops any previous call's listener
    confirmBtn.replaceWith(freshBtn);
    freshBtn.addEventListener('click', () => { onConfirm(); closeModal(layer); }, { once: true });

    openModal(layer);
  }

  /** Grade/class delete picker. The shell (icon, title, description,
   *  section labels, footer buttons) is static markup in index.html
   *  (#j-modal-delete-structure) — only the pill lists are genuinely
   *  data-driven, so only they get rebuilt here. */
  function openDeleteStructureModal(grade) {
    let selectedFullGrade = false;
    const selectedClasses = new Set();
    const checkIcon = '<svg class="c-icon" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg>';

    const layer = document.getElementById('j-modal-delete-structure');
    ...
    // Fresh confirm button each open, so a previous open's listener can't stack.
    const oldConfirmBtn = document.getElementById('j-del-structure-confirm');
    const confirmBtn = oldConfirmBtn.cloneNode(true);
    oldConfirmBtn.replaceWith(confirmBtn);

    function render() {
      gradeWrap.innerHTML = `<button type="button" class="c-del-pill j-del-pill-grade${selectedFullGrade ? ' c-is-selected' : ''}">${selectedFullGrade ? checkIcon : ''} Full ${escapeHtml(grade.name)}</button>`;

      classWrap.innerHTML = grade.classes.map((c) => {
        const isSel = selectedClasses.has(c);
        return `<button type="button" class="c-del-pill j-del-pill-class${isSel ? ' c-is-selected' : ''}" data-class="${escapeHtml(c)}">${isSel ? checkIcon : ''} Class ${escapeHtml(c)}</button>`;
      }).join('');

      confirmBtn.disabled = !selectedFullGrade && selectedClasses.size === 0;

      gradeWrap.querySelector('.j-del-pill-grade').addEventListener('click', () => {
        selectedFullGrade = !selectedFullGrade;
        if (selectedFullGrade) selectedClasses.clear();
        render();
      });
      classWrap.querySelectorAll('.j-del-pill-class').forEach((btn) => {
        btn.addEventListener('click', () => {
          const c = btn.dataset.class;
          if (selectedClasses.has(c)) selectedClasses.delete(c);
          else { selectedClasses.add(c); selectedFullGrade = false; }
          render();
        });
      });
    }
    ...
    confirmBtn.addEventListener('click', () => { executeDeletion(); closeModal(layer); });
